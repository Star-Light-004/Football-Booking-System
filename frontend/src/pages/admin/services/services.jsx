...
export default function AdminServices() {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      const res = await axios.get("http://127.0.0.1:8000/api/services/list/?admin=true");
      setServices(res.data);
      
      // Fetch fields for mapping
      const fieldsRes = await axios.get("http://127.0.0.1:8000/api/football-fields/");
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa dịch vụ này không?")) {
      try {
        await axios.delete(`http://127.0.0.1:8000/api/services/list/${id}/`);
...
function ServiceModal({ service, availableFields, onClose, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (service) {
        await axios.put(`http://127.0.0.1:8000/api/services/list/${service.id}/`, formData, config);
        alert("Cập nhật thành công");
      } else {
        await axios.post(`http://127.0.0.1:8000/api/services/list/`, formData, config);
